...
#include "handbot_mtc_pick_place_demo/get_planning_scene_client.h"
...
: Node("get_planning_scene_client")
{
  ...
  client_ = create_client<handbot_interfaces::srv::GetPlanningScene>("/get_planning_scene_handbot");
}
...
GetPlanningSceneClient::call_service(const std::string& target_shape, const std::vector<double>& target_dimensions)
{
  ...
  // Prepare the request
  auto request = std::make_shared<handbot_interfaces::srv::GetPlanningScene::Request>();
